...
def main(args):
    ...
    s = 0
    total_msa_count = len(seqs)  # 计算总共的msa数量
    ...
    while(s < len(seqs)):
        ...
        time_per_msa = (current_time - start_time) / (e / chunk_size)
        estimated_remaining_time = (total_msa_count - e) * time_per_msa
        # 输出当前进度和估计剩余时间
        print(f"已处理 {e}/{total_msa_count} MSA，估计剩余时间: {estimated_remaining_time:.2f} 秒")
